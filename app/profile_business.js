    ...
    return new Promise((resolve, reject) => {
      uploadTask.on('state_changed',
        ...
        async () => {
          ...
          const userDocRef = doc(db, 'business_users', auth.currentUser.uid);
...
const validateForm = () => {
  let valid = true;
  let newErrors = {};
  const requiredFields = ['stallName', 'location', 'openingHours', 'contactNumber', 'email'];
...
const handleUpdate = async () => {
  ...
  if (!await handlePasswordUpdate(formData.password)) {
    return; 
  }
